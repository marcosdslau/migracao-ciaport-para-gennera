...
const EnrollmentPosRepository = require('../repository/EnrollmentPosRepository');
...
class EnrollmentService {
    ...
    async Registra(id_student, id_enrollment, name, code, academic_responsible_person_cpf, financial_responsible_person_document, academic_calendar, course, curriculum, module, clazz, shift, status, id_campaign, campaign, polo, date_enrollment, start_date, cancellation_reason, cancellation_date, observation, id_parent_enrollment){
        let beginTransaction = await connection.transaction();
        let subjectRepository = new EnrollmentPosRepository(beginTransaction);
        try {
            let alreadExist = await subjectRepository.BuscarPeloId(id_enrollment);
            if(!alreadExist) await subjectRepository.criarRegistro(id_student, id_enrollment, name, code, academic_responsible_person_cpf, financial_responsible_person_document, academic_calendar, course, curriculum, module, clazz, shift, status, id_campaign, campaign, polo, date_enrollment, start_date, cancellation_reason, cancellation_date, observation, id_parent_enrollment);
            else await subjectRepository.atualizarRegistro(id_student, id_enrollment, name, code, academic_responsible_person_cpf, financial_responsible_person_document, academic_calendar, course, curriculum, module, clazz, shift, status, id_campaign, campaign, polo, date_enrollment, start_date, cancellation_reason, cancellation_date, observation, id_parent_enrollment);
    ...
    async BuscaTodos(){
        let subjectRepository = new EnrollmentPosRepository();
        try {
